/*
# RoutesRed — Full Schema Rebuild (v2 with corrections)

## Overview
This migration replaces the entire `routesred` schema with a corrected,
security-hardened version. The previous 11 migrations (20260827002649 through
20260827005351) created the initial schema but had several security gaps.

**This migration is written for review only.** After approval it will be
applied via `apply_migration`.

## Corrections in v2
1. Storage policies use correct storage.foldername() semantics:
   [1]='providers', [2]=pid, [3]=resource_type, [4]=resource_id
2. All storage EXISTS use single AND block wrapping all OR branches.
3. Nested IDOR validates vehicle/driver belongs to provider from [2].
4. Buckets truly idempotent: INSERT if not exists, UPDATE if exists.
5. provider_documents INSERT protected: status must be 'pending',
   no reviewed_by/reviewed_at/rejection_reason on insert.
6. Document validation checks applies_to, provider_types coherence.
7. create_provider(): individual requires first_name AND last_name.
8. Slug generator checks reserved words and auto-generates valid variant.
9. vehicles: capacity > 0, luggage_capacity >= 0, year range 1900-2100.
10. Seeds: RFC and tax status apply to individual AND company.
11. Platform RPCs only operate on platform='routesred'.
12. All SECURITY DEFINER functions get REVOKE FROM PUBLIC, anon.
13. GRANT USAGE on schema + GRANT SELECT on catalog tables for anon.
14. Views are internal-only; public interface is RPCs exclusively.
*/
...
-- Grant USAGE so anon/authenticated can access schema objects
...
-- 3c. airports (PostGIS lives in extensions schema)
...
-- 3d. document_types (redesigned)
-- Seeds corrected: RFC proof and tax status apply to individual AND company.
-- Company-exclusive documents are separated.
...
DROP TRIGGER IF EXISTS document_types_updated_at ON routesred.document_types;
...
DROP POLICY IF EXISTS "document_types_public_read" ON routesred.document_types;
...
-- Column-level protection: REVOKE direct INSERT, UPDATE, DELETE
...
-- anon gets NO SELECT on transport_providers (public read is via RPC only)

-- Defensive trigger: block changes to protected fields unless super_admin
...
DROP TRIGGER IF EXISTS trg_protect_provider_fields ON routesred.transport_providers;
...
-- Mutations only via RPC: REVOKE direct INSERT, UPDATE, DELETE
...
-- Protective trigger: prevent leaving provider with zero active owners
...
DROP TRIGGER IF EXISTS trg_protect_last_owner ON routesred.transport_provider_users;
...
-- All get REVOKE FROM PUBLIC, anon before GRANT to authenticated.
-- =============================================================
...
-- Correction: individual requires first_name AND last_name (not OR).
-- Correction: slug generator checks reserved words.
-- =============================================================
...
BEGIN
  ...
  -- Individual: first_name AND last_name both required
  ...
  -- Check reserved words and auto-generate valid variant
  v_slug := v_base;
...
-- Correction: only operate on platform='routesred'.
-- No naturestayred, no arbitrary registration_source.
-- =============================================================
...
-- =============================================================
-- 13. Vehicles
-- Correction: capacity > 0, luggage_capacity >= 0, year 1900-2100
-- =============================================================
...
-- anon gets NO SELECT (public read via RPC)
...
DROP TRIGGER IF EXISTS trg_vehicle_images_updated_at ON routesred.vehicle_images;
...
DROP TRIGGER IF EXISTS trg_ensure_single_primary ON routesred.vehicle_images;
...
-- Storage integrity: bucket must be routesred-public, path under providers/{pid}/vehicles/{vid}/
...
DROP TRIGGER IF EXISTS trg_validate_vehicle_image_storage ON routesred.vehicle_images;
...
DROP POLICY IF EXISTS "vehicle_images_select_members" ON routesred.vehicle_images;
...
DROP POLICY IF EXISTS "vehicle_images_insert_members" ON routesred.vehicle_images;
...
DROP POLICY IF EXISTS "vehicle_images_update_members" ON routesred.vehicle_images;
...
DROP POLICY IF EXISTS "vehicle_images_delete_members" ON routesred.vehicle_images;
...
-- =============================================================
-- 15. vehicle_amenities (join table)
-- =============================================================
...
DROP POLICY IF EXISTS "vehicle_amenities_select_members" ON routesred.vehicle_amenities;
...
DROP POLICY IF EXISTS "vehicle_amenities_insert_members" ON routesred.vehicle_amenities;
...
DROP POLICY IF EXISTS "vehicle_amenities_delete_members" ON routesred.vehicle_amenities;
...
-- Corrections:
-- - INSERT trigger: status must be 'pending', no admin fields on insert.
-- - Validation trigger: checks document_types.applies_to, provider_types,
--   vehicle_id/driver_id coherence with applies_to.
-- =============================================================
...
DROP TRIGGER IF EXISTS trg_pd_updated_at ON routesred.provider_documents;
...
-- Trigger: block INSERT with non-pending status or admin fields
...
DROP TRIGGER IF EXISTS trg_protect_doc_insert ON routesred.provider_documents;
...
-- Trigger: block UPDATE of admin fields
...
DROP TRIGGER IF EXISTS trg_protect_doc_admin ON routesred.provider_documents;
CREATE TRIGGER trg_protect_doc_admin
  BEFORE UPDATE ON routesred.provider_documents
  FOR EACH ROW EXECUTE FUNCTION routesred.protect_document_admin_fields();

-- Validation trigger: applies_to, provider_types, vehicle/driver coherence
...
BEGIN
  ...
  -- Check provider_types coherence
  ...
  -- Storage integrity: must use routesred-private bucket
  ...
  -- Check applies_to coherence with vehicle_id / driver_id + storage path
...
DROP TRIGGER IF EXISTS trg_validate_provider_document ON routesred.provider_documents;
...
-- INSERT policy: permissions depend on applies_to of the document_type
-- provider docs: owner/administrator only
-- vehicle/driver docs: owner/administrator/operator_manager
...
-- =============================================================
-- 18. provider_agency_links (conditional on public.agencies existing)
-- =============================================================
DO $do_pal$
BEGIN
...
-- =============================================================
-- 20. Public read RPCs (SECURITY DEFINER)
-- anon cannot SELECT directly from transport_providers or vehicles.
-- These RPCs return only public DTO columns.
-- =============================================================
...
-- =============================================================
-- 21. Views with security_invoker = true (internal use only)
-- NOT the public interface. Public reads go through RPCs.
-- =============================================================
...
-- Only authenticated can use internal views
...
-- =============================================================
-- 22. Storage: idempotent bucket create/update + policy replacement
-- Correction: truly idempotent (INSERT if not exists, UPDATE if exists).
-- Correction: storage.foldername() semantics: [1]='providers', [2]=pid,
--   [3]=resource_type, [4]=resource_id
-- Correction: single AND block wrapping all OR branches.
-- =============================================================

-- Idempotent bucket creation: insert if not exists
...
-- 22a. Public bucket: SELECT (anon + authenticated)
DROP POLICY IF EXISTS "routesred_public_select" ON storage.objects;
CREATE POLICY "routesred_public_select"
  ON storage.objects FOR SELECT
  TO anon, authenticated
  USING (bucket_id = 'routesred-public');

-- 22b. Public bucket: INSERT
-- Path layout: providers/{pid}/profile/...     -> owner, administrator
--              providers/{pid}/vehicles/{vid}/ -> owner, admin, operator_manager
--              providers/{pid}/drivers/{did}/  -> owner, admin, operator_manager
-- storage.foldername(objects.name) returns: [1]=providers, [2]=pid, [3]=type, [4]=id
DROP POLICY IF EXISTS "routesred_public_insert" ON storage.objects;
    ...
    AND EXISTS (
      ...
      WHERE tpu.user_id = auth.uid()
        ...
        AND (
          ...
          (
            (storage.foldername(objects.name))[1] = 'providers'
            AND (storage.foldername(objects.name))[2] = tpu.transport_provider_id::text
            AND (storage.foldername(objects.name))[3] = 'profile'
            AND tpu.role IN ('owner', 'administrator')
          )
          OR (
            -- Vehicles: owner, admin, operator_manager + nested IDOR
            (storage.foldername(objects.name))[1] = 'providers'
            AND (storage.foldername(objects.name))[2] = tpu.transport_provider_id::text
            AND (storage.foldername(objects.name))[3] = 'vehicles'
            AND tpu.role IN ('owner', 'administrator', 'operator_manager')
            AND (
              (storage.foldername(objects.name))[4] IS NULL
              OR EXISTS (
                SELECT 1 FROM routesred.vehicles v
                WHERE v.id::text = (storage.foldername(objects.name))[4]
                  AND v.transport_provider_id = tpu.transport_provider_id
              )
            )
          )
          OR (
            -- Drivers: owner, admin, operator_manager + nested IDOR
            (storage.foldername(objects.name))[1] = 'providers'
            AND (storage.foldername(objects.name))[2] = tpu.transport_provider_id::text
            AND (storage.foldername(objects.name))[3] = 'drivers'
            AND tpu.role IN ('owner', 'administrator', 'operator_manager')
            AND (
              (storage.foldername(objects.name))[4] IS NULL
              OR EXISTS (
                SELECT 1 FROM routesred.drivers d
                WHERE d.id::text = (storage.foldername(objects.name))[4]
                  AND d.transport_provider_id = tpu.transport_provider_id
              )
            )
          )
        )
...
-- 22c. Public bucket: UPDATE
DROP POLICY IF EXISTS "routesred_public_update" ON storage.objects;
...
-- 22d. Public bucket: DELETE
DROP POLICY IF EXISTS "routesred_public_delete" ON storage.objects;
...
-- 22e. Private bucket: SELECT
-- Additional resource type: documents (provider-level: owner/admin)
DROP POLICY IF EXISTS "routesred_private_select" ON storage.objects;
        ...
        WHERE tpu.user_id = auth.uid()
          ...
          AND (
            ...
            OR (
              (storage.foldername(objects.name))[1] = 'providers'
              AND (storage.foldername(objects.name))[2] = tpu.transport_provider_id::text
              AND (storage.foldername(objects.name))[3] = 'documents'
              AND tpu.role IN ('owner', 'administrator')
            )
            OR (
              (storage.foldername(objects.name))[1] = 'providers'
...
-- 22f. Private bucket: INSERT
DROP POLICY IF EXISTS "routesred_private_insert" ON storage.objects;
...
-- 22g. Private bucket: UPDATE
DROP POLICY IF EXISTS "routesred_private_update" ON storage.objects;
...
-- 22h. Private bucket: DELETE
DROP POLICY IF EXISTS "routesred_private_delete" ON storage.objects;
...
-- =============================================================
-- 23. Audit — deferred
-- TODO: RoutesRed ecosystem audit integration
-- Wait for shared migration to add 'routesred' to tenant_type enum.
-- No parallel audit table. No tenant_type='system' for RoutesRed events.
-- =============================================================
